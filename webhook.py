...
def embed(discord_username, discord_id, steam_id, prefix, action, color, ip):
    ...
    if env["app"]["app-get-user-ip"] == True:
...
async def new_user_added(discord_id, discord_username, steam_id, ip):

    webhook = Webhook.from_url(
        env["app"]["app-webhook-url"], adapter=RequestsWebhookAdapter())

    webhook.send(embed=embed(discord_username, discord_id,
                 steam_id, "Linked", "verified", 0x00ff00, ip), username=env["app"]["app-name"]+"-hook")


async def unlink_msg(discord_id, discord_username, steam_id, ip):
    webhook = Webhook.from_url(
        env["app"]["app-webhook-url"], adapter=RequestsWebhookAdapter())

    webhook.send(embed=embed(discord_username, discord_id,
                 steam_id, "Unlinked", "unlinked", 0xff0000, ip), username=env["app"]["app-name"]+"-hook")
